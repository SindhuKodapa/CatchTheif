...
secret_key = 'YYYYYY'
token = 'ZZZZZ'
eval_image_dir = "/Users/Downloads/2_challenge_eval"
eval_annotation_file = "/Users/sindhu/Downloads/output_labels_challenge2_latest.json"
tmp_s3_bucket= "custom-labels-console-us-east-1-877028bf95" # this bucket needs to exist
rek_endpoint_uri = "arn:aws:rekognition:us-east-1:848643745672:project/metro/version/metro.2024-10-13T11.06.32/1728831992426"
'''
end constants
'''
...
for record in ground_truth:
    f = join(eval_image_dir, record["filename"])
    with open(f, 'rb') as image:
        

        #transfer to s3
        unique_name = str(uuid.uuid4())+".png"
        s3 = boto3.client('s3', region_name = 'us-east-1', aws_access_key_id = access_key, aws_secret_access_key = secret_key, aws_session_token = token)
...
positive_images = set( record["filename"] for record in ground_truth)
neg_images = [join(eval_image_dir, f) for f in listdir(eval_image_dir) if isfile(join(eval_image_dir, f)) and f not in positive_images and f.endswith(".png")]
